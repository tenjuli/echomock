...
if __name__ == "__main__":
  ...
  # Windowのハンドル取得('クラス名','タイトルの一部')で検索クラスが不明なら、
  # Noneにする。
  ...
  # ちなみに、アプリ内のボタンとか入力窓も頑張ればとれるけど、win32guiでやると
  # 複雑になりすぎる。
  ...
  if parent_handle > 0 :
    ...
    # ウインドウの完全な情報を取ってくる、FindWindowで部分一致だったりした
    # 場合の補完用
    titlebar = win32gui.GetWindowText(parent_handle)
    classname = win32gui.GetClassName(parent_handle)

  # Drag/Drop関数があるけれど、実はmouseDownとmoveToを使ったほうが良い。
  # アプリのマウスオーバー挙動は D&D では反応しない(逆に反応させない様に
  # 使う場合はdragTo/dragRelがいい)
